...
const enemyInfo = { name:true, hero:true, weapon:true, helmet:true, armor:true };
...
// 敌方设置
function toggleEnemyCfg() {
  document.getElementById('enemyCfgPop').classList.toggle('open');
}
function onEnemyFilterChange() {
  enemyInfo.name   = document.getElementById('chkEnemyName').checked;
  enemyInfo.hero   = document.getElementById('chkEnemyHero').checked;
  enemyInfo.weapon = document.getElementById('chkEnemyWeapon').checked;
  enemyInfo.helmet = document.getElementById('chkEnemyHelmet').checked;
  enemyInfo.armor  = document.getElementById('chkEnemyArmor').checked;
  _scheduleCanvasRender();
}

// 盒子筛选
let _showPlayerBox = true, _showAIBox = false;
function toggleBoxCfg() {
  document.getElementById('boxCfgPop').classList.toggle('open');
}
function onBoxFilterChange() {
  _showPlayerBox = document.getElementById('chkPlayerBox').checked;
  _showAIBox = document.getElementById('chkAIBox').checked;
  _scheduleCanvasRender();
}
document.addEventListener('click', e => {
  const boxPop = document.getElementById('boxCfgPop');
  if (boxPop && boxPop.classList.contains('open') && !e.target.closest('#boxCfgPop') && !boxPop.parentElement.contains(e.target)) {
    boxPop.classList.remove('open');
  }
  const enemyPop = document.getElementById('enemyCfgPop');
  if (enemyPop && enemyPop.classList.contains('open') && !e.target.closest('#enemyCfgPop') && !enemyPop.parentElement.contains(e.target)) {
    enemyPop.classList.remove('open');
  }
});
