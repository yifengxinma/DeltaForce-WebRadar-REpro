...
function _renderAll() {
  ...
  if (layerState.container) (data.containers || []).forEach(c => {
    ...
    _drawText(distLabel(c.x, c.y, c.z||0), pt.x, pt.y + sz/2 + 8*S, `600 ${Math.round(8*S)}px sans-serif`, col);
  ...
  if (layerState.item) {
    ...
    _clusters.forEach(cluster => {
      ...
      // 直接在Canvas上列出物品（最多3个，多余省略）
      const MAX_SHOW = 3;
      const lineH = 11 * S;
      const fSize = Math.round(8 * S);
      let startY = pt.y - sz/2 - 2 - Math.min(cluster.length, MAX_SHOW) * lineH;
      ...
      shown.forEach(({it: ci}) => {
        ...
        const zD = Math.round(((ci.z||0) - (_spectateRef.z||0)) / 100);
        const zArrow = zD > 0 ? '↑'+zD : zD < 0 ? '↓'+Math.abs(zD) : '';
        const label = (ci.n||'物品') + (cvStr ? ' '+cvStr : '') + (zArrow ? ' '+zArrow : '');
        _drawText(label, pt.x, startY, `600 ${fSize}px sans-serif`, cc);
        startY += lineH;
      });
      // 不再显示 "...+N" 省略提示
      // 距离（图标下方）
      let below = pt.y + sz/2 + 2;
      _drawText(distLabel(it.x, it.y, it.z||0), pt.x, below, `600 ${fSize}px sans-serif`, col);
  ...
  function _drawPlayer(p) {
    ...
    if (_isEnemy) {
      ...
      // 护甲行（独立控制）
      if (helmetLv || armorLv) {
        _cx.font = `600 ${Math.round(9*S)}px sans-serif`;
        let parts = [];
        if (helmetLv) {
          const hTxt = (p.hh!=null&&p.hm) ? Math.round(p.hh)+'/'+Math.round(p.hm) : String(helmetLv);
          parts.push({type:'helmet', lv:helmetLv, txt:hTxt});
        }
        if (armorLv) {
          const aTxt = (p.ah!=null&&p.am) ? Math.round(p.ah)+'/'+Math.round(p.am) : String(armorLv);
          parts.push({type:'armor', lv:armorLv, txt:aTxt});
        }
        let totalW = 0;
        parts.forEach((pp, i) => {
          totalW += 14*S + _cx.measureText(pp.txt).width;
          if (i > 0) totalW += 6*S;
        });
        let cx = pt.x - totalW/2;
        parts.forEach((pp, i) => {
          if (i > 0) cx += 6*S;
          if (pp.type === 'helmet') _drawHelmetIcon(cx + 6*S, above, pp.lv);
          else _drawArmorIcon(cx + 6*S, above, pp.lv);
          cx += 14*S;
          _drawText(pp.txt, cx + _cx.measureText(pp.txt).width/2, above, `600 ${Math.round(9*S)}px sans-serif`, lvColor(pp.lv), 'center');
          cx += _cx.measureText(pp.txt).width;
        });
        above -= 12*S;
      }
      // 名字行（各项独立控制: 名称/干员/手持）
      {
        let nameStr = '';
        if (hero) nameStr += '[' + hero + ']';
        if (_ei.name && p.n && p.n !== 'DefaultName') nameStr += p.n;
        if (weapon) nameStr += '[' + weapon + ']';
        if (nameStr) _drawText(nameStr, pt.x, above, `bold ${Math.round(10*S)}px sans-serif`, fill);
      }
